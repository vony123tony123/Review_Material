...
public abstract class Animal extends JLabel implements Runnable {
	...
	protected static int shark_num[];
	...
	public static void connectNum(int[] fish, int[] turtle, int[] shark) {
		fish_num = fish;
		turtle_num = turtle;
		shark_num = shark;
	}
		...
		public void mouseClicked(MouseEvent e) {
			if (removeswitch) {
				...
				if (e.getSource().getClass().getName() == "Fish") {
					fish_num[0]--;
				} else if(e.getSource().getClass().getName() == "Turtle"){
					turtle_num[0]--;
				}else if(e.getSource().getClass().getName() == "Shark") {
					shark_num[0]--;
				}
				Main.UpdateStatus();
			}
